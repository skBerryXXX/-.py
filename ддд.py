...
# meta developer: @SenkoGuardianModules cli by @SKBERRYXXX
...
GEMINI_TIMEOUT = 840
GEMINI_CLI_CREDS_PATH = os.path.expanduser("~/.gemini/oauth_credentials.json")
...
# requires: google-genai google-api-core google-auth google-auth-httplib2 pytz markdown_it_py

class Gemini(loader.Module):
    """Модуль для работы с Google Gemini AI через Gemini CLI OAuth. (Поддержка видео/фото/аудио)"""
    strings = {
        "name": "Gemini",
        "cfg_api_key_doc": "Путь к файлу credentials Gemini CLI. По умолчанию: ~/.gemini/oauth_credentials.json",
        ...
        "no_api_key": (
            '❗️ <b>Gemini CLI credentials не найдены.</b>\n'
            'Установите Gemini CLI: <code>npm install -g @google/gemini-cli</code>\n'
            'Затем войдите: <code>gemini auth login</code>\n'
            'Credentials будут сохранены в <code>~/.gemini/oauth_credentials.json</code>\n'
            'Или укажите путь вручную: <code>.cfg Gemini cli_creds_path /путь/к/файлу.json</code>'
        ),
        ...
        "new_sdk_missing": "⚠️ <b>Для работы модуля нужны библиотеки google-genai и google-auth.</b>\nВыполните: <code>pip install google-genai google-auth google-auth-httplib2</code>",
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            loader.ConfigValue("cli_creds_path", "", self.strings["cfg_api_key_doc"]),
        ...
        self.api_keys = []
        self._cli_credentials = None  # кэшированные OAuth credentials из Gemini CLI

    async def client_ready(self, client, db):
        self.client = client
        self.db = db
        self.me = await client.get_me()
        self.api_keys = []  # не используется в CLI-режиме
        ...
        # Загружаем OAuth credentials при старте
        try:
            self._cli_credentials = await asyncio.to_thread(self._load_cli_credentials_sync)
        except Exception as e:
            logger.warning(f"Gemini CLI credentials не загружены: {e}")
    ...
    async def _send_to_gemini(self, message, parts: list, regeneration: bool=False, call: InlineCall=None, status_msg=None, chat_id_override: int=None, impersonation_mode: bool=False, use_url_context: bool=False, display_prompt: str=None, attempt: int = 1, is_retry: bool = False, ephemeral: bool = False): 
        ...
        if provider == "openrouter":
            ...
            try:
                ...
                return None
        # Загружаем OAuth credentials из Gemini CLI
        try:
            cli_creds = await self._get_cli_credentials()
        except Exception as _cred_err:
            err_msg = (
                f"❗️ <b>Gemini CLI credentials недоступны.</b>\n"
                f"<code>{utils.escape_html(str(_cred_err))}</code>\n\n"
                "Войдите командой: <code>gemini auth login</code>"
            )
        ...
        _t_start = time.time()
        # Один запрос через Gemini CLI OAuth credentials (без ротации ключей)
        for i in range(1):
            try:
                http_opts = None
                if proxy_config:
                    http_opts = types.HttpOptions(async_client_args={"proxies": proxy_config})
                client = genai.Client(credentials=cli_creds, http_options=http_opts)
    ...
    async def gmusic(self, message: Message):
        ...
        try:
            cli_creds = await self._get_cli_credentials()
            client = genai.Client(credentials=cli_creds)
    ...
    async def gskey(self, message: Message):
        """— Проверить статус аутентификации Gemini CLI."""
        m = await utils.answer(message, "🔐 <b>Проверка Gemini CLI credentials...</b>")
        try:
            creds = await self._get_cli_credentials()
            token_preview = (creds.token or "")[:12] + "..."
            expiry_str = str(getattr(creds, "expiry", "неизвестно"))
            report = (
                f"✅ <b>Gemini CLI авторизован</b>\n"
                f"🔑 <b>Token:</b> <code>{utils.escape_html(token_preview)}</code>\n"
                f"⏱ <b>Истекает:</b> <code>{utils.escape_html(expiry_str)}</code>\n"
                f"📂 <b>Файл:</b> <code>{utils.escape_html(self.config.get('cli_creds_path') or GEMINI_CLI_CREDS_PATH)}</code>"
            )
        except Exception as e:
            report = (
    ...
    def _load_cli_credentials_sync(self):
        """Загружает OAuth credentials из файла Gemini CLI (синхронно, для asyncio.to_thread)."""
        if not GOOGLE_OAUTH_AVAILABLE:
            raise RuntimeError("google-auth не установлен. Выполните: pip install google-auth google-auth-httplib2")
        creds_path = self.config.get("cli_creds_path", "").strip() or GEMINI_CLI_CREDS_PATH
        if not os.path.exists(creds_path):
            raise FileNotFoundError(
                f"Файл credentials не найден: {creds_path}\n"
                "Войдите в Gemini CLI: gemini auth login"
            )
        with open(creds_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        creds = google_oauth2_credentials.Credentials(
        ...
        if creds.expired and creds.refresh_token:
            ...
            try:
                ...
                with open(creds_path, "w", encoding="utf-8") as fw:
                    json.dump(data, fw, indent=2)
            except Exception:
                pass
        return creds

    async def _get_cli_credentials(self):
        """Возвращает актуальные OAuth credentials, обновляя при необходимости."""
        if not GOOGLE_OAUTH_AVAILABLE:
            raise RuntimeError("google-auth не установлен")
        try:
            creds = await asyncio.to_thread(self._load_cli_credentials_sync)
            self._cli_credentials = creds
            return creds
        except Exception as e:
    ...
    def _handle_error(self, e: Exception) -> str:
        ...
        if isinstance(e, RuntimeError) and "Все ключи исчерпали квоту" in str(e):
             return self.strings["api_error"].format(utils.escape_html(str(e)))
        ...
        if "quota" in msg.lower() or "429" in msg: return self.strings["api_error"].format(utils.escape_html(msg))
    ...
    async def _scan_keys(self, force=False):
        """Проверяет валидность Gemini CLI credentials (совместимость)."""
        if not GOOGLE_AVAILABLE:
            return "Library missing", []
        try:
            creds = await self._get_cli_credentials()
            return f"✅ <b>Gemini CLI авторизован.</b>\nToken: {(creds.token or '')[:10]}...", []
        except Exception as e:
            return f"❗️ <b>Ошибка:</b> {utils.escape_html(str(e))}", []

    def _get_sorted_keys(self):
        """Заглушка для совместимости — в CLI-режиме ключи не используются."""
        return []

    async def _call_google_rest(self, model_name: str, prompt: str, input_image_bytes=None):
        """REST-запрос к Gemini API через Gemini CLI OAuth Bearer token."""
        try:
            creds = await self._get_cli_credentials()
            access_token = creds.token
        except Exception as e:
            return {"error": {"message": f"OAuth credentials error: {e}"}}
        if not access_token:
            return {"error": {"message": "OAuth token пуст. Выполните: gemini auth login"}}
        parts = [{"text": prompt}]
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(url, json=payload, headers=headers, proxy=proxy, timeout=60) as resp:
                    if resp.status == 200:
                        return await resp.json()
                    elif resp.status == 401:
                        # Токен мог истечь — пробуем обновить
                        try:
                            creds = await self._get_cli_credentials()
                        ...
                        return {"error": {"message": f"HTTP 401: Unauthorized. {text[:200]}"}}
                    else:
                        text = await resp.text()
                        return {"error": {"message": f"HTTP {resp.status}: {text[:200]}"}}
            except Exception as e:
